          ...
          <div className="flex flex-col justify-center space-y-4">
            ...
            <div className="space-y-4">
              <div className="flex items-center gap-2">
                ...
                <span>{t("feature1") || "Професійний ремонт"}</span>
              </div>
              <div className="flex items-center gap-2">
                <CheckCircle className="h-5 w-5 text-primary" />
                <span>{t("feature2") || "Гарантія на всі роботи"}</span>
              </div>
              <div className="flex items-center gap-2">
                <CheckCircle className="h-5 w-5 text-primary" />
                <span>{t("feature3") || "Швидке обслуговування"}</span>
              </div>
            </div>
            <div className="flex flex-col gap-2 min-[400px]:flex-row mt-4">
              <Link href="/contact" passHref>
                <Button size="lg" className="px-8">
                  {t("requestService") || "Замовити ремонт"}
                  <ArrowRight className="ml-2 h-4 w-4" />
                </Button>
              </Link>
              <Link href="/services" passHref>
                <Button size="lg" variant="outline" className="px-8">
                  {t("viewServices") || "Переглянути послуги"}
                </Button>
              </Link>
            </div>
          </div>
          <div className="relative h-[300px] md:h-[400px] lg:h-[500px] xl:h-[600px] rounded-2xl overflow-hidden shadow-xl">
            ...
            {isImageLoaded && (
              <Image
                ...
                alt={t("imageAlt") || "Ремонт мобільного телефону"}
                fill
