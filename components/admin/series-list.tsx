...
export function SeriesList({ brandId }: SeriesListProps) {
  ...
  async function handleAddSeries() {
    ...
    try {
      ...
      setNewSeries({ name: "", brandId: brandId || "" })
  ...
  return (
    <div className="space-y-6">
      {!brandId && (
        <div className="flex items-center justify-between">
          <div>
            ...
            <p className="text-muted-foreground">{t("manageSeries") || "Manage device series"}</p>
          </div>
          <div className="flex gap-2">
            <Button variant={isReorderMode ? "default" : "outline"} onClick={() => setIsReorderMode(!isReorderMode)}>
              <MoveVertical className="mr-2 h-4 w-4" />
              {isReorderMode ? t("doneReordering") : t("reorderSeries") || "Reorder Series"}
            </Button>
      ...
      <Card>
        <CardHeader>
          <CardTitle>{t("series") || "Series"}</CardTitle>
          <CardDescription>{t("seriesDescription") || "Manage device series for your brands"}</CardDescription>
          {!brandId && (
            <div className="mt-4">
                                  ...
                                  <DropdownMenu>
                                    ...
                                    <DropdownMenuContent align="end">
                                      ...
                                      <DropdownMenuItem asChild>
                                        <Link href={`/admin/series/${item.id}/models`}>
                                          <Smartphone className="mr-2 h-4 w-4" />
                                          {t("viewModels") || "View Models"}
                                        </Link>
                                      </DropdownMenuItem>
                                      <DropdownMenuSeparator />
                                      <DropdownMenuItem
      ...
      <Dialog open={isAddDialogOpen} onOpenChange={setIsAddDialogOpen}>
        <DialogContent>
          <DialogHeader>
            ...
            <DialogDescription>
              {t("addNewSeriesDescription") || "Add a new device series to a brand"}
            </DialogDescription>
          </DialogHeader>
      ...
      <Dialog open={isEditDialogOpen} onOpenChange={setIsEditDialogOpen}>
        <DialogContent>
          <DialogHeader>
            ...
            <DialogDescription>{t("editSeriesDescription") || "Edit device series details"}</DialogDescription>
          </DialogHeader>
          {editSeries && (
            <div className="grid gap-4 py-4">
              <div className="grid gap-2">
      ...
      <Dialog open={isDeleteDialogOpen} onOpenChange={setIsDeleteDialogOpen}>
        <DialogContent>
          <DialogHeader>
            ...
            <DialogDescription>{t("deleteSeriesDescription") || "This action cannot be undone"}</DialogDescription>
          </DialogHeader>
          <div className="py-4">
            <p>
              {t("deleteSeriesConfirmation", { series: seriesToDelete?.name }) ||
                `Are you sure you want to delete "${seriesToDelete?.name}"?`}
            </p>
          </div>
          <DialogFooter>
