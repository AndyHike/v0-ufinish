  ...
  const clearSession = async () => {
    try {
      ...
      if (data.success) {
        ...
        setStatus("error")
        setMessage("Не вдалося очистити сесію. Спробуйте ще раз або зверніться до адміністратора.")
      }
    } catch (error) {
      setStatus("error")
  ...
  return (
    <div className="container flex items-center justify-center min-h-screen py-12">
      <Card className="w-full max-w-md">
        <CardHeader>
          <CardTitle>Відновлення доступу</CardTitle>
          <CardDescription>Очищення сесії для відновлення доступу до сайту</CardDescription>
        </CardHeader>
        <CardContent>
          {status === "loading" && (
