...
export default async function SeriesPageClient({ params }: Props) {
            ...
            <div className="grid grid-cols-2 gap-6 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5">
              {models.map((model) => (
                <Link
                  ...
                  onClick={() => {
                    // Додаємо клієнтський скрипт для відстеження кліку
                    if (typeof window !== "undefined") {
                      // Для Facebook Pixel
                      ;(window as any).fbq?.("track", "ViewContent", {
                        content_name: model.name,
                        content_type: "model",
                        content_category: `${series.brands?.name} - ${series.name}`,
                      })

                      // Для Google Analytics
                      ;(window as any).gtag?.("event", "view_model", {
                        event_category: "Models",
                        event_label: `${series.brands?.name} - ${series.name} - ${model.name}`,
                        brand: series.brands?.name,
                        series: series.name,
                        model: model.name,
                      })
                    }
