...
export async function POST(request: NextRequest) {
  try {
    // Check if REMONLINE_API_TOKEN is set
    if (!process.env.REMONLINE_API_TOKEN) {
      console.error("REMONLINE_API_TOKEN is not set in environment variables")
      return NextResponse.json({ error: "REMONLINE_API_TOKEN is not set in environment variables" }, { status: 500 })
    }
    ...
    // Verify the webhook secret if RemOnline provides one
